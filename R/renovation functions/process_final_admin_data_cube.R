...
process_final_admin_data_cube <- function(validated_int_data_cube, context) {
     
  ## Input validation (just check validated_int_data_cube is at least bigger than raw data) 
  if (nrow(validated_int_data_cube) < nrow(context$raw_data)) {
    stop("Validated data cube is smaller than raw data cube. This is a problem.")
  }
   
  ## Cache data cube and local context
  validated_int_data_cube %>% arrow::write_parquet(context$path_cache_staged_data)
  ...
  {  #### Metadata Template
    ...
    query <- glue_sql("
      SELECT DISTINCT {`context$vec__admin_metadata_composite_keys`*}
      FROM parquet_scan({context$path_cache_staged_data})
    ", .con = con)
    metadata_cube_key_template <- dbGetQuery(con, query) %>% as_tibble()
    metadata_cube_key_template %>% write_parquet(context$path_cache_metadata_cube_key_template)
  }
  
  ## Return
  cli_alert_success(glue("Cached validated final data cube at {context$path_cache_staged_data}"))
