...
The complexity of cube building is that there are different types of relationships between data points and metadata fields. For the cube building process the first step of metadata is to clearly for each field define what composite keys are used to link the metadata to the data points.

Please open linkage.csv and enter this information. In our case this is a **medium complexity** dataset:
  
- Most of the metadata exists at a dataset level
- A few fields exist at a variable level:
  - var_label
   - var_def
   - value_type
   - units
   - coding
- **KEY DIFFERENCE**: Some fields vary by country (iso2 level):
  - source (different data sources per country)
  - acknowledgements (country-specific acknowledgements)

This demonstrates **medium complexity** where metadata differs by geographic composite keys, requiring country-specific metadata management.
