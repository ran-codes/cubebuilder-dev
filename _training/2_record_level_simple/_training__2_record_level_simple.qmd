...
{ ## 'Global Setup (For any notebook)'
  ...
  # Add location where files for the current dataset version will be written to. Example: 'datasets/area-level/LE_L1/v1.0'
  setwd(here::here('code/renovations/_training/_training__2_record_level_simple'))
...
{ ## Renovation notebook specific setup
  ...
  local_config = lst(
    ...
    dataset_id_tmp = "_training__2_record_level_simple", # Dataset ID name. Example: 'LE_L1'
    ...
    raw_dataset_dir = 'Dashboards/_training/_training__2_record_level_simple', # Unencrypted server data location. Example: 'Data/Mortality Data/Life Expectancy/'
  ...
  ## Develop data freeze files (Create a frozen copy of data files)
  file.path(global_context$path_server_dmc, local_config$raw_dataset_dir) %>%
      list.files() %>% 
      discard(~str_detect(.x, 'Codebook_')) # Specify the prefix for codebooks. This excludes codebook(s) from being frozen.
  
  #### Generic renovation notebook setup code (Do not change!)
  local_context = get_renovation_notebook_context(local_config, global_context) 
...
- Raw data: freezes raw data into long term stable storage and compiles raw data into `local_context$raw_data`
...
df_var_name = tibble(var_name_raw = local_context$raw_data %>% 
                       select(-all_of(
  ...
  rowwise() %>% 
  mutate(
    var_name = paste0(local_context$dataset_id_tmp, '_', sanitize_codebook_var(var_name_raw)),
  ...
  ungroup() %>% 
  select(all_of(local_context$vec__admin_variable_definition_table_columns)) %>% 
  arrange(var_name)
...
process_variable_table(df_var_name, local_context)
```
...
df_strata = generate_empty_df_strata(df_var_name, local_context)
...
```{r}
#| code-summary: 'Automated - QC'
process_strata_table(df_strata, local_context)
```
...
```{r}
if (!file.exists(local_context$path_cache_int_data)){
  
  df_data_raw = local_context$raw_data %>% collect() %>% as_tibble()
  glimpse(df_data_raw)
  ...
  df_data_int = df_data_raw %>%  
    ...
    ## selections
    select(
      all_of(local_context$vec__admin_composite_keys_all),
      all_of(local_context$vec__admin_data_columns_never_empty),
      any_of(local_context$vec__admin_data_columns_all)
    )
  
  ## Cache preliminary results to enable out-of-memory work
  df_data_int %>% write_parquet(local_context$path_cache_int_data)
  rm(df_data_int)
  gc()
...
```{r}
if (file.exists(local_context$path_cache_staged_data)) {
  final_data_cube = open_dataset(local_context$path_cache_staged_data)
} else {
  
  ## Validate
  df_data_int = local_context$path_cache_int_data %>% 
    open_dataset() %>% 
    collect()
  validated_int_data_cube = df_data_int %>% 
    validate_final_admin_data_cube(., local_context)
  gc()
  
  ## Process 
  final_data_cube = validated_int_data_cube %>% 
    process_final_admin_data_cube(., local_context)
  gc()
...
```{r}
## Process
template_metadata_cube = process_linkage_table(local_context)
...
if (file.exists('4.1-raw__codebook.xlsx')){ 
  raw_codebook_object = import_salurbal_xslx_cdbk(local_context$raw_cdbk_path, local_context)
  raw_codebook_object 
}
```
...
valid_raw__cdbks = raw_codebook_object %>% 
  process_raw_metadata_object(., local_context)
...
```{r}
process_preliminary_admin_cube(local_context)
```
...
## Validate Prelim OBT
valid_prelim_admin_cube = local_context %>% 
