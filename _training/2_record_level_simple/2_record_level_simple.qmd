...
df_var_name = tibble(var_name_raw = context$raw_data %>% 
                       select(-c(file_data, schema_version, dataset_version, dataset_id,
                                 observation_type, dataset_instance,
                                 # Remove composite key columns
                                 RESPONDENT_ID, SURVEY_YEAR, SURVEY_MONTH, SURVEY_DAY)) %>% 
                       names()) %>% 
  ## Variable sanitation
  rowwise() %>% 
...
if (!file.exists(context$path_cache_int_data)){
  ...
  df_data_int = df_data_raw %>%
    ...
    select(-RESPONDENT_ID, -SURVEY_YEAR, -SURVEY_MONTH, -SURVEY_DAY) %>%
    ## Pivot longer to reshape health survey measurements
    pivot_longer(
      cols = c(-file_data, -schema_version, -dataset_version, -dataset_id,
               -observation_type, -dataset_instance, -observation_id,
               -iso2, -geo, -year, -month, -day),
...
No we run through our validation function that checks for consistency and the correct structure.
...
Great now this data cube is validated. Let's move on to the metadata.

# 2. Metadata Standardization

## 2.1 Setup Metadata Template

The complexity of SALURBAL is that there are different types of relationships between data points and metadata fields. For the SALURBAL renovation the first step of metadata is to clearly for each field define what composite keys are used to link the metadata to the data points.
...
Once the linkage.csv is clearly defined. We can use the linkage.csv to generate a metadata template that is compatible with the data cube and is as normalized as possible for ease of metadata entry. 
...
Great now our template is set up. We can see a few new files in our folder: 

- template__codebook files are the template for metadata entry. This is just a backup of the original template for reference.
- raw__codebook. this is the template that renovators first fill out before it is processed and reviewed. 
...
TIP: You can copy codebooks and the template and have AI fill it out. It works pretty well.
...
- Denormalize: turn the normalized sheets into a metadata cube that can be easily joined with the data cube
